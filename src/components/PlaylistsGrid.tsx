  ...
  if (vibe.gifUrl) {
    return (
      <div className="absolute inset-0 z-10 flex items-center justify-center overflow-hidden bg-black/60 backdrop-blur-xs">
        ...
        <img src={vibe.gifUrl} alt={vibe.title} className="h-full w-full object-cover" />
      </div>
    );
  }
...
function OnboardingWizardModal({ playlist, onComplete, onClose }: OnboardingWizardProps) {
  ...
  const macroGenre = useMemo(() => detectMacroGenre(playlist.name), [playlist.name]);
  const subgenreList = useMemo(() => SUBGENRE_DICTIONARY[macroGenre] || SUBGENRE_DICTIONARY.chill, [macroGenre]);
  const namingProposals = useMemo(() => generateSmartNamingProposals(playlist.name, macroGenre), [playlist.name, macroGenre]);
  ...
  const handleStep1Select = (type: PlaylistClassification) => {
    ...
    // Solo las playlists Objetivo pasan al Paso 2 de configuración profunda de ADN
      ...
      >
        <AnimatePresence mode="wait">
          {step === 1 ? (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-semibold text-emerald-400">Asistente ADN →</span>
                    </div>
                    <p className="text-xs text-white/40 leading-snug mt-0.5">Destino curado con calidad benchmark. Asistente de ADN, subgéneros y naming.</p>
                  </div>
                </button>
            ...
            /* ── Step 2: Enriched Subgenres & Smart Naming ── */
            <motion.div
              key="step2"
              initial={{ opacity: 0, x: 16 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              {/* ── Sección A (ADN): 10 Subgéneros Enriquecidos ── */}
              <div className="flex flex-col gap-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-white/40 font-mono">
                    Género: <span className="text-white/80 font-bold uppercase">{macroGenre}</span> ({selectedSubgenres.length} elegidos)
                  </span>
                </div>
              ...
              <div className="flex flex-col gap-2.5 pt-2 border-t border-white/5">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-white/80 uppercase tracking-wider flex items-center gap-1.5">
                    <span>🏷️ Sección B: Nombre Estandarizado</span>
                  </label>
